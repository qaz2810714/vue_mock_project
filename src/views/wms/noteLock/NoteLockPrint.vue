<template>
   ...
   <div>
       <div class="cont" id="printDiv">
        <div class="title">锁定单</div>
        <div class="contractInfor clearfix">
            <ul class="fl">
                <li><label>锁定单号：</label><span>{{mainInfo.lockNo}}</span></li>
                <li><label>仓库：</label><span>{{whsInfo.name}}</span></li>
                <li><label>锁定时间：</label><span>{{mainInfo.auditTime}}</span></li>
            </ul>
            <ul class="fr">
                <li><label>锁定状态：</label><span>{{mainInfo.status}}</span></li>
...
export default {
    ...
    data:function(){
        return{
            config:{
                mainEntity:metadata.wmsNoteLock,
                detailEntity:metadata.wmsNoteLockDetail,
                dicCache:null
            },
            mainInfo:{},
            whsInfo:{},
    ...
    methods: {
        ...
        initData(entity){
            //明细单的转换
            let _this = this;
            this.detailInfo = entity.detailList.map(item =>_this.getDisplayModel(item,_this.config.detailEntity));
            //主单的转换
