<template>
   ...
   <div>
       <div class="cont" id="printDiv">
        <div class="title">移库单</div>
        <div class="contractInfor clearfix">
            <ul class="fl">
                <li><label>移库单号：</label><span>{{mainInfo.moveNo}}</span></li>
                <li><label>仓库：</label><span>{{mainInfo.whsInfo.name}}</span></li>
                <li><label>移库时间：</label><span>{{mainInfo.createTime}}</span></li>
            </ul>
            <ul class="fr">
                <li><label>移库状态：</label><span>{{mainInfo.moveStatus}}</span></li>
...
export default {
    ...
    data:function(){
        return{
            ...
            mainInfo:{whsInfo:{name:''},oldStack:'',newStack:''},
            detailInfo:[],
            //打印状态
            isPrinting:false,
    ...
    methods: {
        ...
        getDisplayModel(data,entity){
            ...
            oldStack = componentUtil.getStackName(this.config.dicCache,data,'originRoomId','originAreaId');
            if(data['originStackName']){
                oldStack = oldStack + ">" + data['originStackName'];
            }
            if(data['originLayer']){
                oldStack = oldStack + ">" + data['originLayer'];
            }
            newStack = componentUtil.getStackName(this.config.dicCache,data,'targetRoomId','targetAreaId');
            if(data['targetStackName']){
                newStack = newStack + ">" + data['targetStackName'];
            }
            if(data['targetLayer']){
                newStack = newStack + ">" + data['targetLayer'];
            }
            result['oldStack'] = oldStack;
            result['newStack'] = newStack;
            return result;
        },
        initData(entity){
            //明细单的转换
